...
```
+----------------------------------------------+
|  Jeel جيل       [ Login ]  [ Sign Up ]       |
+----------------------------------------------+
|  Find the right center for your child        |
|  [ Search by activity, location... ] [Search]|
|  [ All ] [ Sports ] [ Arts ] [ Coding ]      |
+----------------------------------------------+
|  +----------------+  +----------------+      |
|  | Nujoom Academy |  | CodeKids       |      |
|  | Riyadh · Sports|  | Jeddah · Coding|      |
|  | ⭐ 4.8         |  | ⭐ 4.5         |      |
|  +----------------+  +----------------+      |
+----------------------------------------------+
```

#### Screen 3 — Center Details

```
+----------------------------------------------+
|  [Logo]  Nujoom Academy                      |
|          Riyadh · ⭐ 4.8 (42 reviews)        |
|          [Sports] [Arts] [Ages 5–14]         |
+----------------------------------------------+
|  Programs              | Book a Program      |
|  Swimming · 450 SAR    | [Select program ▼]  |
|  Basketball · 380 SAR  | [Child's age   ▼]   |
|                        | [ Book Now ]        |
+----------------------------------------------+
```
...
```
+----------------------------------------------+
|  Book a Program — Nujoom Academy             |
+----------------------------------------------+
|  Program:   [ Swimming — Beginners      ▼ ]  |
|  Child Age: [ 6 years                   ▼ ]  |
|  Date:      [ Select date...            ▼ ]  |
|  Price:     450 SAR/month                    |
|             [ Confirm Booking ]              |
...
#### Screen 5 — Center Dashboard

```
+--------------------+-------------------------+
|  Dashboard (active)|  Welcome back 👋        |
|  My Programs       |  Bookings: 87           |
|  Bookings          |  Programs: 5            |
|  Reviews           |  Rating: 4.8 ⭐         |
|  Profile           |-------------------------|
|  Settings          |  Sarah M. · Swimming ✅ |
|                    |  Noura T. · Drawing  ⏳ |
+--------------------+-------------------------+
```
...
1. The Frontend (React.js / Next.js) sends HTTP requests to the Backend API.
1. The Backend (Node.js / Express) processes the request and queries the Database.
1. PostgreSQL returns data to the Backend, which sends it as JSON to the Frontend.
1. The Frontend renders the response to the user.
...
|Table   |Fields                                                                                             |
|--------|---------------------------------------------------------------------------------------------------|
|users   |id (PK), name, email, password, role (parent/center/admin)                                         |
|centers |id (PK), name, location, description, approved, owner_id (FK → users)                              |
|courses |id (PK), name, age_range, price, center_id (FK → centers)                                          |
|bookings|id (PK), user_id (FK → users), course_id (FK → courses), date, status (pending/confirmed/cancelled)|
|reviews |id (PK), user_id (FK → users), centre_id (FK → centers), rating (1–5), comment                     |
...
|------------------|--------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|React.js / Next.js|Frontend      |Team has experience with HTML/CSS/JS — React is a natural extension. Next.js adds SSR for better performance and SEO.                                                                      |
|Node.js / Express |Backend       |Same language as Frontend (JavaScript) — reduces learning complexity. Express is lightweight and ideal for building REST APIs.                                                             |
|PostgreSQL        |Database      |Data is structured with clear relationships (Users, Centers, Bookings) — Relational DB is more suitable than NoSQL.                                                                        |
|JWT               |Authentication|Stateless auth — no session storage needed. Supports role-based access for 3 user types (Parent, Center, Admin).                                                                           |
|Google Maps API   |External      |Most accurate maps for Saudi Arabia. Easy integration. Required by User Story: filter by location.                                                                                         |
|Cloudinary        |External      |Provides CDN + image optimization with no extra infrastructure. Lets the team focus on development.                                                                                        |
|GitHub            |SCM           |Team is familiar with it from coursework. Supports Issues, PRs, and Code Review — everything needed for team collaboration.                                                                |
|Docker            |Environment   |Ensures the project runs the same way on all devices (Windows, Mac, Linux) — eliminates “works on my machine” issues. Essential for a 4-person team working on different operating systems.|
