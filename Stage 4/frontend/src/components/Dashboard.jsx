...
const TYPE_COLORS = {
  "برمجة": "#3b82f6","فن": "#ec4899","رياضة": "#10b981",
  "لغات": "#f59e0b","علوم": "#06b6d4","رياضيات": "#ef4444",
  "طبخ": "#f97316","أخرى": "#64748b",
};
...
function SettingsSection({ center }) {
  ...
  const handleUploadLicense = async () => {
    ...
    try {
      ...
      const res = await API.post('/upload/document', form, { headers:{'Content-Type':'multipart/form-data'} });
    ...
    } catch (err) { alert("فشل رفع الترخيص: "+(err.response?.data?.message||err.message)); }
  };
    ...
    try {
      ...
      if (settingLogo) {
        try {
          ...
          alert("فشل رفع الصورة: "+(uploadErr.response?.data?.message||uploadErr.message));
          return;
        }
      }
      await API.patch(`/centers/${center.id}`, { name:settingName, location:settingCity, description:settingDesc, image:imageUrl });
      setSettingSuccess("تم حفظ التغييرات بنجاح.");
      setTimeout(() => setSettingSuccess(""), 3000);
    } catch (err) { alert("فشل حفظ التغييرات: "+(err.response?.data?.message||err.message)); }
  };
  ...
  return (
    <div className="cd-section">
      <div className="cd-page-header"><div><h1>الإعدادات</h1><p>إدارة إعدادات مركزك</p></div></div>
      ...
      <div className="cd-card cd-settings-item" onClick={()=>setShowPasswordForm(!showPasswordForm)}>
        <div className="cd-settings-item-right">
          <div className="cd-settings-item-icon" style={{background:"#fef9c3"}}><FiSettings style={{color:"#a16207"}} /></div>
          <div><strong>تغيير كلمة المرور</strong><span>تحديث كلمة مرور حسابك</span></div>
        </div>
      ...
      <div className="cd-card cd-settings-item" onClick={()=>setShowLicenseForm(!showLicenseForm)}>
        <div className="cd-settings-item-right">
          <div className="cd-settings-item-icon" style={{background:"#e0f2fe"}}><FiBookOpen style={{color:"#0369a1"}} /></div>
          <div><strong>الترخيص</strong><span>إدارة ترخيص وموافقات المركز</span></div>
        </div>
        ...
        <div className="cd-card cd-settings-sub">
          {center?.license_file && (
            <div className="cd-license-existing">
              ...
              <a href={center.license_file} target="_blank" rel="noreferrer" className="cd-license-link">عرض الترخيص الحالي</a>
            </div>
          )}
          ...
          {licenseFile && (
            <button className="cd-btn-primary" style={{marginTop:"12px"}} onClick={handleUploadLicense}>رفع الملف</button>
          )}
        </div>
      )}
    </div>
...
function Dashboard() {
  ...
  useEffect(() => {
    if (role==="admin") {
      Promise.all([API.get("/centers/all"), API.get("/bookings/all")])
        .then(([cRes,bRes]) => { setCenters(cRes.data); setAllBookings(bRes.data); setLoading(false); })
        .catch(()=>setLoading(false));
    }
    if (role==="center") {
      Promise.all([API.get("/centers/mine"), API.get("/bookings/center")])
        .then(([cRes,bRes]) => {
          setCenter(cRes.data); setCenterBookings(bRes.data);
          if (cRes.data?.id) API.get(`/centers/${cRes.data.id}/courses`).then(r=>setCourses(r.data)).catch(()=>{});
          setLoading(false);
        })
        .catch(()=>setLoading(false));
    }
  ...
  const handleApprove = async (id) => {
    try { await API.patch(`/centers/${id}/approve`); setCenters(centers.map(c=>c.id===id?{...c,approved:true}:c)); }
    catch { alert("فشل اعتماد المركز."); }
  };
  const handleUnapprove = async (id) => {
    try { await API.patch(`/centers/${id}`,{approved:false}); const r=await API.get("/centers/all"); setCenters(r.data); }
    catch { alert("فشل إلغاء اعتماد المركز."); }
  };
  const handleDeleteCenter = async (id) => {
    try { await API.delete(`/centers/${id}`); const r=await API.get("/centers/all"); setCenters(r.data); }
  ...
  const handleSubmitCenter = async (e) => {
    e.preventDefault();
    try {
      let imageUrl=null, licenseUrl=null;
      if (image) {
        const f=new FormData(); f.append('image',image);
        const r=await API.post('/upload',f,{headers:{'Content-Type':'multipart/form-data'}});
        imageUrl=r.data.url;
      }
      if (license) {
        const f=new FormData(); f.append('document',license);
        const r=await API.post('/upload/document',f,{headers:{'Content-Type':'multipart/form-data'}});
        licenseUrl=r.data.url;
      }
      const res=await API.post('/centers',{name:centerName,location,description,image:imageUrl,license_file:licenseUrl});
      setCenter(res.data);
  ...
  const handleRejectBooking = (id) => {
    askConfirm("هل أنت متأكد من رفض هذا الحجز؟", async()=>{
      try {
        await API.patch(`/bookings/${id}/status`,{status:"cancelled"});
        setCenterBookings(centerBookings.filter(b=>b.id!==id));
        showToast("success","تم رفض الحجز.");
  ...
  const buildTimeString = (start, duration) => {
    ...
    let hour=parseInt(h), isAM=period==="ص";
  ...
  // ══ إضافة دورة — بدون duration ══
  const handleAddCourse = async (e) => {
    e.preventDefault();
    try {
      const res=await API.post("/courses",{
        name:courseName, description:courseType, instructor:courseInstructor,
        times:buildTimeString(courseTimeStart,courseTimeDuration),
        days:courseDays, price:coursePrice, center_id:center.id,
      });
      setCourses([...courses,res.data]);
      setCourseName(""); setCourseInstructor(""); setCourseType("");
      setCourseTimeStart(""); setCourseTimeDuration("");
      setCoursePrice(""); setCourseDays("");
  ...
  const handleOpenEdit = (course) => {
    ...
    setEditTimeStart(timeParts[0]||""); setEditTimeDuration("");
    setEditPrice(course.price||""); setEditDays(course.days||"");
  };

  // ══ تعديل دورة — بدون duration ══
  ...
  // ══ فورم الدورة (مشترك بين إضافة وتعديل) ══
  ...
  const TimeHours = ["8:00 ص","9:00 ص","10:00 ص","11:00 ص","12:00 م","1:00 م","2:00 م","3:00 م","4:00 م","5:00 م","6:00 م","7:00 م","8:00 م","9:00 م"];
    ...
    <div className="cd-root" dir="rtl">
      ...
      <aside className={`cd-sidebar ${sidebarOpen?"open":"closed"}`}>
        <div className="cd-sidebar-logo">
          ...
          <button className="cd-menu-btn" onClick={()=>setSidebarOpen(!sidebarOpen)}><FiMenu /></button>
        </div>
        {sidebarOpen && (
        ...
        <button className="cd-nav-item cd-home-link" onClick={()=>navigate("/")} title={!sidebarOpen?"الصفحة الرئيسية":""}>
        ...
        <nav className="cd-nav">
          {navItems.map(item=>(
            <button key={item.key} className={`cd-nav-item ${activeSection===item.key?"active":""}`}
              onClick={()=>setActiveSection(item.key)} title={!sidebarOpen?item.label:""}>
      ...
      <main className="cd-main">
        ...
        {role==="admin" && (
          <>
            <div className="cd-page-header"><div><h1>لوحة الإدارة</h1><p>إدارة المراكز والحجوزات</p></div></div>
            ...
            <div className="cd-admin-tabs">
              <button className={`cd-admin-tab ${adminTab==="centers"?"active":""}`} onClick={()=>{setAdminTab("centers");setActiveSection("centers");}}>المراكز</button>
              <button className={`cd-admin-tab ${adminTab==="bookings"?"active":""}`} onClick={()=>{setAdminTab("bookings");setActiveSection("bookings");}}>الحجوزات</button>
            </div>
            {activeSection!=="bookings" && (
              <div className="cd-card">
                <div className="cd-card-header"><h2>قائمة المراكز</h2><span className="cd-count-badge">{centers.length} مركز</span></div>
                {centers.length===0 ? <p className="cd-empty">لا توجد مراكز بعد</p> : (
                      ...
                      <div key={c.id} className="cd-admin-center-card">
                        ...
                        <div className="cd-admin-center-info">
                          ...
                          <div className="cd-admin-docs" style={{marginTop:8}}>
                            {c.license_file
                              ? <a href={c.license_file} target="_blank" rel="noreferrer" className="cd-doc-link"><FiFileText style={{fontSize:13}}/> عرض الترخيص / السجل</a>
                              : <span className="cd-doc-missing"><FiAlertCircle style={{fontSize:13}}/> لم يُرفع ترخيص بعد</span>
                            }
                          </div>
                        </div>
                        <div className="cd-admin-center-actions">
                          <span className={`cd-badge ${c.approved?"confirmed":"pending"}`}>{c.approved?"معتمد":"قيد المراجعة"}</span>
                          <div className="cd-actions" style={{marginTop:"8px"}}>
                            {!c.approved && <button className="cd-btn-approve" onClick={()=>handleApprove(c.id)}>اعتماد</button>}
                            {c.approved && <button className="cd-btn-undo" onClick={()=>handleUnapprove(c.id)}>إلغاء الاعتماد</button>}
            ...
            {activeSection==="bookings" && (
              <div className="cd-card">
                <div className="cd-card-header"><h2>جميع الحجوزات</h2><span className="cd-count-badge">{allBookings.length} حجز</span></div>
                <table className="cd-table">
                  <thead><tr><th>المركز</th><th>الدورة</th><th>المستخدم</th><th>التاريخ</th><th>الحالة</th></tr></thead>
                  <tbody>
        ...
        {role==="center" && (
          <>
            {!center && (
              <div className="cd-section">
                <div className="cd-page-header"><div><h1>إضافة مركز جديد</h1><p>أدخل بيانات مركزك لإرسالها للمراجعة</p></div></div>
            ...
            {center && center.approved && (
              <>
                ...
                {activeSection==="home" && (
                  <div className="cd-section">
                    <div className="cd-page-header"><div><h1>مرحباً، {center.name}</h1><p>إدارة مركزك ودوراتك وحجوزاتك</p></div></div>
                    ...
                    <div className="cd-home-grid">
                      <div className="cd-card">
                        <div className="cd-card-header"><h2>أحدث الحجوزات</h2><button className="cd-view-all" onClick={()=>setActiveSection("bookings")}>عرض الكل</button></div>
                        <table className="cd-table">
                      ...
                      <div className="cd-card">
                        ...
                        <div className="cd-courses-list">
                          {courses.length===0 ? <p className="cd-empty">لا توجد دورات مضافة حالياً</p>
                ...
                {activeSection==="courses" && (
                  <div className="cd-section">
                    ...
                    {/* ══ فورم الإضافة — عمودي بدون مدة الدورة ══ */}
                    {showCourseForm && !editingCourse && (
                      <div className="cd-card cd-form-card">
                        <h3 style={{marginBottom:16}}>إضافة دورة جديدة</h3>
                        <form onSubmit={handleAddCourse} className="cd-form">
                          <div className="cd-form-row-2">
                          ...
                          <div className="cd-time-picker">
                            ...
                            <div className="cd-time-row">
                              <select className="cd-input cd-select" value={courseTimeStart} onChange={e=>setCourseTimeStart(e.target.value)} required>
                                ...
                                {TimeHours.map(h=><option key={h} value={h}>{h}</option>)}
                              </select>
                    ...
                    {/* ══ فورم التعديل — عمودي بدون مدة الدورة ══ */}
                    {editingCourse && (
                      <div className="cd-card cd-form-card cd-edit-card">
                        ...
                        <form onSubmit={handleSaveEdit} className="cd-form">
                          <div className="cd-form-row-2">
                          ...
                          <div className="cd-time-picker">
                            ...
                            {editTimeStart && (
                              <span className="cd-time-preview">
                                {editTimeDuration ? buildTimeString(editTimeStart,editTimeDuration) : editTime}
                              </span>
                            )}
                          </div>
                          <div className="cd-form-row-2">
                    ...
                    <div className="cd-card">
                      <table className="cd-table">
                        <thead><tr><th>الدورة</th><th>المدرب</th><th>التصنيف</th><th>تاريخ البدء</th><th>الوقت</th><th>السعر</th><th>الإجراءات</th></tr></thead>
                        <tbody>
                          {courses.length===0
                            ...
                            : courses.map(course=>(
                              <tr key={course.id}>
                                ...
                                <td>{course.days ? new Date(course.days).toLocaleDateString("ar-SA") : "-"}</td>
                                <td>{course.times}</td>
                ...
                {activeSection==="bookings" && (
                  <div className="cd-section">
                    <div className="cd-page-header"><div><h1>الحجوزات</h1><p>عرض الحجوزات الخاصة بدورات المركز</p></div></div>
                  ...
                  <div className="cd-section">
                    ...
                    <div className="cd-card cd-profile-card">
                      <div className="cd-profile-header">
                        ...
                        <div><h2>{center.name}</h2><p>{center.location}</p><span className="cd-badge confirmed">معتمد</span></div>
                      </div>
                ...
                {activeSection==="settings" && <SettingsSection center={center} />}
              </>
            )}
          </>
        )}
        ...
        <div className="cd-modal-overlay" onClick={()=>setConfirmModal(null)}>
          <div className="cd-modal" onClick={e=>e.stopPropagation()}>
            ...
            <div className="cd-modal-actions">
              <button className="cd-btn-primary" onClick={()=>{confirmModal.onConfirm();setConfirmModal(null);}}>تأكيد</button>
